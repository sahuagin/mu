...
enum Command {
    ...
    /// Probe provider APIs for per-model output/context limits (mu-1gx5).
    ///
    /// Reads what providers REPORT (openrouter max_completion_tokens, ollama
    /// architectural context_length) and writes them to models.probed.toml,
    /// which the catalog layers below your hand-edited models.toml — so the
    /// per-request max_tokens stops relying on drift-prone prefix rules.
    Models {
        #[command(subcommand)]
        cmd: ModelsCmd,
    },
...
#[derive(Subcommand, Debug)]
enum ModelsCmd {
    /// Probe providers and (re)write ~/.config/mu/models.probed.toml.
    Refresh {
        /// Providers to probe (comma-separated). Default: openrouter,ollama
        /// (the providers that expose a capability API today).
        #[arg(long)]
        providers: Option<String>,
        /// Output file. Default: ~/.config/mu/models.probed.toml.
        #[arg(long, value_name = "PATH")]
...
async fn run_models(cmd: ModelsCmd) -> Result<()> {
    match cmd {
        ModelsCmd::Refresh { providers, output } => run_models_refresh(providers, output).await,
    }
}

/// mu-1gx5: probe each provider's models API and (re)write models.probed.toml.
/// Best-effort per provider — a provider that can't be built (missing key/OAuth)
/// or whose probe fails is logged and skipped, not fatal: the file still gets
/// every provider that DID answer. The catalog layers this below models.toml.
async fn run_models_refresh(
    providers: Option<String>,
    output: Option<std::path::PathBuf>,
) -> Result<()> {
    use mu_coding::serve::factory::{build_provider_from_selector, selector_from_cli};
    use mu_core::context::CacheTtl;
    use mu_core::model_catalog::{
        probed_config_path, probed_models_to_catalog, ModelCatalogConfig,
    };

    // Default to the providers that expose a capability API today.
    let names: Vec<String> = providers
        ...
        .unwrap_or_else(|| vec!["openrouter".to_string(), "ollama".to_string()]);

    let mut catalog = ModelCatalogConfig::default();
    for name in &names {
        let provider = match selector_from_cli(name, None)
            .and_then(|sel| build_provider_from_selector(&sel, true, None, CacheTtl::default()))
        {
            Ok(p) => p,
            Err(e) => {
                eprintln!("mu models refresh: skip {name}: {e}");
                continue;
            }
        };
        match provider.probe_model_capabilities().await {
            Ok(models) => {
                let frag = probed_models_to_catalog(&models);
                eprintln!(
                    "mu models refresh: {name}: {} model(s) with reported limits",
                    frag.models.len()
                );
                catalog.models.extend(frag.models);
            }
            Err(e) => eprintln!("mu models refresh: {name}: probe failed: {e}"),
        }
    }

    let path = output
        .or_else(probed_config_path)
        .context("no config dir for models.probed.toml")?;
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .with_context(|| format!("creating {}", parent.display()))?;
    }
    let banner = "# GENERATED by `mu models refresh` (mu-1gx5) — DO NOT EDIT.\n\
                  # Provider-reported model limits. Layered BELOW your models.toml,\n\
                  # so any hand override there wins. Re-run to refresh.\n\n";
    let body = toml::to_string_pretty(&catalog).context("serializing probed catalog to TOML")?;
    std::fs::write(&path, format!("{banner}{body}"))
        .with_context(|| format!("writing {}", path.display()))?;
    let n = catalog.models.len();
    println!(
        "wrote {n} model entr{} to {}",
        if n == 1 { "y" } else { "ies" },
        path.display()
    );
    Ok(())
