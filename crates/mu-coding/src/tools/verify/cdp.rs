...
/// Largest single CDP frame we will buffer. A screenshot is a few hundred
/// KB of base64; anything past this is an untrusted page shouting through
/// `console.log` and is discarded whole (counted as a dropped event).
pub const FRAME_MAX_BYTES: usize = 16 * 1024 * 1024;
...
impl Cdp {
    ...
    pub async fn launch(launcher: &Launcher) -> anyhow::Result<Self> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                frame.clear();
                match read_frame_bounded(&mut reader, &mut frame, FRAME_MAX_BYTES).await {
        ...
        tokio::spawn(async move {
            // Bounded at the source: past 16 KB the launcher's stderr is
            // simply not read further (EOF), so a runaway line can't grow.
            let mut reader = BufReader::new(stderr_pipe.take(16 * 1024));
...
/// Read one NUL-terminated frame into `frame`, never buffering more than
/// `cap` bytes: an oversized frame is skipped to its terminator and
/// reported as [`Frame::Oversized`] instead of growing memory.
async fn read_frame_bounded<R: tokio::io::AsyncBufRead + Unpin>(
    reader: &mut R,
    frame: &mut Vec<u8>,
    cap: usize,
) -> std::io::Result<Frame> {
    ...
    loop {
        ...
        let (chunk, done) = match buf.iter().position(|b| *b == 0) {
            Some(i) => (&buf[..i], true),
            None => (buf, false),
        };
        if !oversized {
            if frame.len() + chunk.len() > cap {
                oversized = true;
                frame.clear();
            } else {
...
mod tests {
    ...
    #[tokio::test]
    async fn oversized_frames_are_skipped_not_buffered() {
        let big = vec![b'x'; 5000];
        let mut data = b"{\"id\":1}\0".to_vec();
        data.extend_from_slice(&big);
        data.push(0);
        data.extend_from_slice(b"{\"id\":2}\0");
        let mut reader = BufReader::with_capacity(64, std::io::Cursor::new(data));
        let mut frame = Vec::new();
        assert!(matches!(
            read_frame_bounded(&mut reader, &mut frame, 1024)
                .await
                .unwrap(),
            Frame::Ok
        ));
