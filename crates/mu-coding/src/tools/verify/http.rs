            ...
            loop {
                tokio::select! {
                    ...
                    accepted = listener.accept() => match accepted {
                        Ok((stream, _)) => {
                            // Never park the accept loop on a permit: at the
                            // cap, a new connection is dropped at once, so a
                            // flood cannot block shutdown or later accepts.
                        ...
                        // Transient accept errors (EMFILE, a reset in the
                        // backlog) must not kill the server mid-probe.
...
mod tests {
    ...
    /// At the connection cap the server keeps accepting (and dropping)
    /// and still shuts down promptly — it never parks on a permit.
    #[tokio::test]
    async fn connection_flood_does_not_block_shutdown() {
        let dir = tempfile::tempdir().unwrap();
        ...
        // A further request is either served or refused — never hangs.
        let path = format!("/{t}/a.html");
        let _ = tokio::time::timeout(Duration::from_secs(3), get(port, &path)).await;
        drop(server);
        tokio::time::sleep(Duration::from_millis(200)).await;
        // Listener is gone: a new connect fails within the bound.
        ...
        assert!(
            ...
            "server must stop listening after drop even under a flood"
        );
        drop(idle);
    }
