...
//! statically known. Permission mirrors `bash`'s: `Allow` in strict and
//! yolo sessions, per-call `Ask` under `--bash-prompt`. Never in a
//! default toolset, so `--tools verify` is the operator's consent to run
//! artifacts (scrubbed environment, own process group, bounded); the
//! one-shot `mu ask` lane cannot answer approval prompts, which is why
//! the default is not `Ask`.
//!
...
fn default_screenshot_dir() -> PathBuf {
    // Per-daemon, not a fixed shared name: a co-tenant must not be able to
    // pre-create or squat the directory another user's screenshots land in.
    std::env::temp_dir().join(format!("mu-verify-{}", std::process::id()))
}
        ...
        ToolSpec {
            ...
            input_schema: json!({
                ...
                "properties": {
                    "artifact": {
                        "type": "string",
                        "description": "Path to the file to run, under the daemon's working directory (the same directory bash commands run in) — absolute or relative; files outside it are refused."
                    },
                    "kind": {
                        "type": "string",
...
/// Resolve the artifact path and require a regular file UNDER the
/// daemon's working directory — the same directory `bash` commands run
/// in. Confinement is the point: the tool runs or serves what the model
/// names, so it may only name what this session's work produced, never
/// an arbitrary pre-existing file elsewhere on disk.
fn resolve_artifact(path: &Path) -> Result<PathBuf, String> {
    ...
    if !canonical.starts_with(&cwd) {
        return Err(format!(
            "verify: artifact must be under the working directory {} (got {}); write the \
             artifact into the workspace and verify it there",
            cwd.display(),
            canonical.display()
        ));
    }
...
mod tests {
    ...
    fn validate_rejects_a_missing_artifact_before_any_prompt() {
        ...
        let f = std::env::current_dir()
            .unwrap()
            .join(format!("mu-verify-validate-{}.js", std::process::id()));
        std::fs::write(&f, "1").unwrap();
        let ok = t.validate(&json!({"artifact": f.display().to_string()}));
    ...
    fn settings_from_config_apply_defaults_and_clamps() {
        ...
        let leaf = s
            ...
            .to_string_lossy()
            .to_string();
        assert!(leaf.starts_with("mu-verify-"), "per-daemon dir: {leaf}");
    }

    #[test]
    fn artifact_outside_the_working_directory_is_refused() {
        let outside = tempfile::Builder::new().suffix(".js").tempfile().unwrap();
        std::fs::write(outside.path(), "1").unwrap();
        // The temp dir is not under the test process's cwd.
        let e = resolve_artifact(outside.path()).unwrap_err();
        assert!(e.contains("must be under the working directory"), "{e}");
    }
