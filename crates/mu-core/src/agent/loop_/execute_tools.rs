...
pub(crate) fn loaded_call_validation_error(spec: &ToolSpec, reason: &str) -> String {
    format!(
...
pub(crate) async fn handle_execute_tools(
    ...
    // name loads it here (load-on-first-call) and runs in the same round.
    ...
    while let Some(call) = calls.pop_front() {
        ...
        // mu-t4l5e: load-on-first-call. A withheld tool is still in `tools`
        ...
        let result = if let Some(cap_reason) = capability_refusal_reason {
            ...
            let content = match (touched, tool) {
                (true, Some(t)) => loaded_call_validation_error(&t.spec(), &reason),
                _ => reason,
            };
            ToolResult {
                content,
