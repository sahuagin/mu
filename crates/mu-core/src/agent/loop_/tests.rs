...
/// mu-bb2v: an operator PreToolUse hook deny must block the tool call
/// BEFORE the `PermissionLevel::Ask` modal fires (same never-ask-about-
/// a-doomed-call principle as mu-bkjr), surface the hook's reason in
/// the ToolResult, and leave the turn completing normally.
#[tokio::test]
async fn mu_bb2v_pre_tool_use_hook_deny_blocks_before_input_required() {
    let provider = mock_provider_one_tool_call("hooked", json!({"x": 1}));
    let tool = MockTool::ok("hooked", "tool would have run but...").with_policy(
        crate::agent::tool::ToolPolicy {
            permission: crate::agent::tool::PermissionLevel::Ask,
            ..Default::default()
        },
    );
    let hooks = crate::hooks::HookEngine::from_config(&crate::config::HooksConfig {
        enabled: true,
        entries: vec![crate::config::HookEntryConfig {
            event: "PreToolUse".to_owned(),
            matcher: Some("hooked".to_owned()),
            command: "cat >/dev/null; echo 'policy: hooked is off-limits' >&2; exit 2".to_owned(),
        ...
        match ev {
            ...
            AgentEvent::ToolCallCompleted {
                ...
                if *is_error
                    ...
                    && content.contains("hooked is off-limits")
                {
                    got_hook_denial = true;
                }
            }
            AgentEvent::Done { .. } => got_done = true,
            _ => {}
    ...
    assert!(
        !got_input_required,
        "InputRequired must NOT fire when a PreToolUse hook denied (mu-bb2v)"
    );
    assert!(
        got_hook_denial,
        "ToolCallCompleted must carry the hook's deny reason"
    );
    assert!(got_done, "Done must fire after the hook denial");
}

/// mu-bb2v counterpart: a PreToolUse hook that allows (exit 0) must be
/// invisible — the tool runs normally.
#[tokio::test]
async fn mu_bb2v_pre_tool_use_hook_allow_is_transparent() {
    let provider = mock_provider_one_tool_call("hooked", json!({"x": 1}));
    let tool = MockTool::ok("hooked", "ran fine under an allowing hook");
    let hooks = crate::hooks::HookEngine::from_config(&crate::config::HooksConfig {
        enabled: true,
        entries: vec![crate::config::HookEntryConfig {
            event: "PreToolUse".to_owned(),
            matcher: None,
            command: "cat >/dev/null; exit 0".to_owned(),
            timeout_ms: Some(5000),
        }],
    })
    .expect("engine");
    let (loop_, events_rx) = spawn_loop(
        provider,
        vec![tool],
        AgentConfig {
            hooks: Some(hooks),
    ...
    let ran_ok = events.iter().any(|e| {
        matches!(
            e,
            AgentEvent::ToolCallCompleted { is_error: false, content, .. }
                if content.contains("ran fine under an allowing hook")
        )
    });
    assert!(ran_ok, "allowing hook must not affect the tool result");
}
