...
enum MockResponse {
    ...
    StartError(String),
    /// mu-049: `Provider::stream` fails with the typed usage cap (the 429
    /// body before any stream) — a rejection, not an accepted request.
    StartUsageLimit(crate::agent::UsageLimit),
...
impl MockProvider {
    ...
    /// mu-049: the first request is rejected with the typed usage cap;
    /// nothing follows (a fallback route answers, or nothing does).
    fn start_usage_limit(limit: crate::agent::UsageLimit) -> Self {
        let mut q = VecDeque::new();
        q.push_back(MockResponse::StartUsageLimit(limit));
        Self {
            responses: Mutex::new(q),
            truncates_over_window: false,
            calls: Default::default(),
...
impl Provider for MockProvider {
    ...
    async fn stream(
        ...
        match chunk {
            ...
            Some(MockResponse::StartUsageLimit(limit)) => Err(ProviderError::UsageLimit(limit)),
...
fn kind(event: &AgentEvent) -> &'static str {
    match event {
        ...
        AgentEvent::AssistantThinkingFinalized { .. } => "assistant_thinking_finalized",
...
async fn b6_provider_error_recoverable() {
    ...
    loop_
        .send(AgentInput::UserMessage(user_msg("try again"), None, None))
        .await
...
async fn mu_779s_iteration_cap_done_event_uses_iteration_cap_stop_reason() {
    ...
    loop_
        .send(AgentInput::UserMessage(user_msg("trigger cap"), None, None))
        .await
        .expect("send");
    let events_handle = tokio::spawn(collect_events(events_rx));
    let _outcome = loop_.join().await;
...
/// mu-frvot: a cap of one has no earlier turn to have spent a budget, so
/// its single turn is an ordinary turn — no preamble, and an answer on it
/// is a plain `EndTurn`, not a cap exit (mu-049's carried-input test
/// depends on this).
#[tokio::test]
async fn mu_frvot_a_cap_of_one_has_no_answer_turn() {
    let provider = MockProvider::new(vec![vec![ProviderEvent::Done(assistant_text("hi"))]]);
    let calls = provider.calls_handle();
    let config = AgentConfig {
        max_turns: Some(1),
        ..AgentConfig::default()
    ...
    assert!(
        !seen[0]
            ...
            .as_ref()
            .is_some_and(|(_, text, _)| text.contains("FINAL TURN")),
        "no preamble on the only turn: {seen:?}"
    );
    let done = events
        .iter()
        .filter_map(|e| match e {
...
/// mu-049 × mu-048: under an armed spend ceiling an IN-STREAM cap came
/// from an accepted request that returned no usage — unknown money: the
/// meter locks (durable `SpendUnaccounted`), no fallback is taken (its
/// call would be refused at preflight anyway), and the cap ends the ask.
/// A REQUEST-TIME cap (the 429 body) is a rejection the server stated:
/// no bill, no lock, and the fallback proceeds.
#[tokio::test]
async fn mu_049_under_a_ceiling_an_accepted_cap_locks_the_meter_and_a_rejected_one_falls_back() {
    let ceiling = || {
        Some(crate::spend::SpendMeter::new(
            crate::spend::SpendCeiling::new(1.0, crate::spend::SpendLanes::Billed).unwrap(),
        ))
    };
    // in-stream: accepted, then capped
    let capped = MockProvider::new(vec![vec![ProviderEvent::UsageLimit(usage_limit(
        "pro", 60,
    ))]]);
    let fallback = MockProvider::new(vec![vec![ProviderEvent::Done(assistant_text("never"))]]);
    let config = AgentConfig {
        spend_meter: ceiling(),
        rate_cards: Some(spend_fixture_catalog(true)),
        fallback_routes: vec![fallback_route(fallback, "anthropic_oauth", "opus")],
    ...
    assert!(
        !events
            .iter()
            .any(|e| matches!(e, AgentEvent::ProviderSwitched { .. })),
        "no fallback once the meter is locked"
    );
    assert!(events.iter().any(|e| matches!(
        e,
        AgentEvent::Done {
    ...
    // request-time: rejected, never accepted
    let rejected = MockProvider::start_usage_limit(usage_limit("pro", 60));
    let fallback = MockProvider::new(vec![vec![ProviderEvent::Done({
        let mut m = assistant_text("from the fallback");
        m.usage = Some(crate::agent::Usage {
            input_tokens: 1_000,
            output_tokens: 0,
            ..Default::default()
    ...
    assert!(
        !events
            .iter()
            .any(|e| matches!(e, AgentEvent::SpendUnaccounted { .. })),
        "a rejection is not a bill"
    );
    assert!(events
        .iter()
        .any(|e| matches!(e, AgentEvent::ProviderSwitched { .. })));
...
async fn b7_user_message_during_tool_pushes_to_back() {
    ...
    loop_
        .send(AgentInput::UserMessage(user_msg("first"), None, None))
        .await
        .expect("send first");
    // Wait for the tool to start, then send a UserMessage.
    tokio::time::sleep(Duration::from_millis(20)).await;
    loop_
        .send(AgentInput::UserMessage(user_msg("second"), None, None))
        .await
        .expect("send second");
...
async fn capability_refuses_tool_outside_allowed_set() {
    ...
    loop_
        .send(AgentInput::UserMessage(user_msg("try blocked"), None, None))
        .await
        .unwrap();
...
async fn kgu4_default_config_does_not_emit_compaction_assembly() {
    ...
    loop_
        .send(AgentInput::UserMessage(user_msg("hello there"), None, None))
        .await
        .expect("send");
    let events_handle = tokio::spawn(collect_events(events_rx));
    let outcome = loop_.join().await;
...
async fn kgu4_evict_half_policy_does_not_fire_when_threshold_not_crossed() {
    ...
    let (loop_, events_rx) = spawn_loop_with_provider(provider, config);
    loop_
        .send(AgentInput::UserMessage(user_msg("hi"), None, None))
        .await
        .expect("send");
    let events_handle = tokio::spawn(collect_events(events_rx));
    let outcome = loop_.join().await;
...
// =============================================================================
// mu-vcbm: per-turn reasoning effort threads the loop → Provider::stream
// =============================================================================

/// One scripted text-only turn, run to completion via the standard
/// send → join → drain harness.
fn effort_loop(
    provider: Arc<dyn Provider>,
    config: AgentConfig,
...
fn one_text_turn() -> Vec<Vec<ProviderEvent>> {
    vec![vec![
        ProviderEvent::TextDelta("ok".into()),
        ProviderEvent::Done(assistant_text("ok")),
    ]]
...
/// The launch-time effort default (`AgentConfig::effort`) seeds the
/// session's standing effort and reaches `Provider::stream` when the ask
/// carries no per-turn override.
#[tokio::test]
async fn vcbm_launch_default_effort_reaches_stream() {
    let (provider, records) = RecordingProvider::new(one_text_turn());
    let (events_tx, events_rx) = mpsc::channel(64);
    let loop_ = effort_loop(
        provider as Arc<dyn Provider>,
        AgentConfig {
            effort: Some(Arc::from("high")),
            ..AgentConfig::default()
        },
        events_tx,
    );

    // No per-turn effort on the ask → the launch default stands.
    loop_
        .send(AgentInput::UserMessage(user_msg("hi"), None, None))
        .await
        .expect("send");
    let events_handle = tokio::spawn(collect_events(events_rx));
    let outcome = loop_.join().await;
    let _ = events_handle.await.expect("events drain");

    assert_eq!(outcome, Outcome::Done(StopReason::EndTurn));
    let records = records.lock().expect("records mutex").clone();
    assert_eq!(records.len(), 1);
    assert_eq!(
        records[0].effort.as_deref(),
        Some("high"),
        "launch-default effort should reach the provider when the ask omits one",
    );
}

/// A per-turn `/effort` change carried on the ask (the UserMessage's
/// third field) overrides the launch default for that turn.
#[tokio::test]
async fn vcbm_per_turn_effort_overrides_launch_default() {
    let (provider, records) = RecordingProvider::new(one_text_turn());
    let (events_tx, events_rx) = mpsc::channel(64);
    let loop_ = effort_loop(
        provider as Arc<dyn Provider>,
    ...
    // The ask carries effort=max → it wins over the launch default.
    loop_
        .send(AgentInput::UserMessage(
            user_msg("go deep"),
            None,
    ...
    assert_eq!(
        records[0].effort.as_deref(),
        Some("max"),
        "per-turn /effort should override the launch default",
    );
}
...
async fn uz0n_no_hint_without_config() {
    ...
    loop_
        .send(AgentInput::UserMessage(user_msg("frobnicate"), None, None))
        .await
        .expect("send");
    let events_handle = tokio::spawn(collect_events(events_rx));
    let _ = loop_.join().await;
