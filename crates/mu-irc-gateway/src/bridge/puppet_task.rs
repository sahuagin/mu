...
//! - **Once registered, it always leaves by QUIT, and always reports
//!   leaving.** The stop reaches it whatever it is parked on (a full command
//!   queue, a report the session is not draining); the QUIT is written on a
//!   line boundary and the task reads on until the SERVER closes — that, and
//!   only that, makes the `Ended` a confirmed departure the session may order
//!   on — bounded by the configured grace, past which the socket is cut and
//!   the `Ended` says so. "Once registered" is load-bearing: before the
//!   welcome there is no session on the server to QUIT from, so a stop, a
//!   timeout, a rejected nick, a failed dial or a closed socket in that window
//!   drops the connection and reports `Ended` with the reason, no QUIT
//!   written. And a CANCEL (see [`Spawn`]) is not a stop at all — it aborts
//!   the task, which writes nothing and reports nothing; the pool that cancels
//!   already knows why.
