...
pub async fn puppet_task(spawn: Spawn) {
    ...
    loop {
        ...
        match reg.on_message(&msg) {
            ...
            Err(AdapterError::NickRejected { numeric, .. }) => {
                ...
                // every other path before registration.
                let told_to_quit = tokio::select! {
                    cmd = cmd_rx.recv() => match cmd {
                        Some(PuppetCommand::Quit { .. }) => true,
                        None => stop_pending(&stop).await,
                        Some(_) => false,
                    },
    ...
    // NICK prefix naming us is compared under from here.
    let cm = reg.isupport().casemapping;
