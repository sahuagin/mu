...
pub struct Membership {
    ...
    /// Names the holder of a RETIRING nick has renamed to on the wire, each
    /// mapped to that retiring nick (folded → folded, with the wire
    /// spellings for a CASEMAPPING change). Until the entry resolves, what
    /// the wire says under the new name is the same story and is held back
    /// under the same entry — the holder answers to the new name now, and
    /// a live event under it must not run ahead of the held-back ones. A
    /// QUIT under it ends the entry like one under the old name.
...
impl Membership {
    ...
    /// The RETIRING entry a folded nick's story is held under: the nick
    /// itself, or the retiring nick its holder renamed from. `None` for a
    /// nick that is owned, or nobody's.
    fn retiring_entry(&self, key: &str) -> Option<String> {
        ...
        self.aliases.get(key).map(|(k, _)| k.clone())
    }

    /// Forget the held-back story of a resolved retiring entry: its arrivals
    /// and the names its holder went by.
    fn forget_story(&mut self, key: &str) {
        self.deferred.remove(key);
        self.aliases.retain(|_, (entry, _)| entry != key);
    }
    ...
    pub fn puppet_departed(&mut self, nick: &str, connection: u64) -> Vec<HumanEffect> {
        ...
        if self
            .retiring
            .get(&key)
            .is_none_or(|held| held.connection != connection)
        {
            return Vec::new();
        }
        self.retiring.remove(&key);
        ...
        let mut effects = Vec::new();
        for arrival in held_back {
            effects.extend(match arrival {
                Arrival::Joined(channel, nick, account) => self.joined(&channel, &nick, account),
                Arrival::Left(channel, nick) => self.left(&channel, &nick),
                Arrival::Renamed(from, to) => self.renamed(&from, &to),
            });
        }
        effects
    }
    ...
    /// puppet behind the report — its JOIN echo to a shared channel may
    /// still arrive after this. The retiring entry is resolved all the same
    /// (the name must not stay ours for good on a server that stopped
    /// answering), but what arrived under it is DISCARDED, not replayed:
    /// it may have been the puppet, and fronting the gateway's own puppet as
    /// a human is the worse mistake. A human who took the name in that
    /// window is picked up by the next snapshot of the channel.
    ...
    pub fn names_reply(
        ...
        for (nick, account) in nicks {
            ...
            let entry = if owned.contains_key(&key) {
                None
            } else if retiring.contains_key(&key) {
                Some(key.clone())
            } else {
                aliases.get(&key).map(|(k, _)| k.clone())
            };
            let puppet = (owned.contains_key(&key) && !gone.contains(&key)) || entry.is_some();
    ...
    pub fn joined(
        ...
        if self.is_own(&key) {
            ...
            if let Some(entry) = self.retiring_entry(&key) {
                self.deferred
                    .entry(entry)
                    .or_default()
                    .push(Arrival::Joined(
    ...
    pub fn quit(&mut self, nick: &str) -> Vec<HumanEffect> {
        ...
        if self.is_puppet(&key) {
            ...
            // naming it (a human's JOIN) is a human's. A QUIT under a name
            // the retiring holder renamed to ends the same entry. For a
            // still-owned puppet it is remembered, so the pool's later
            // release has nothing to wait for.
            match self.retiring_entry(&key) {
                Some(entry) => {
    ...
    ///
    /// A NICK seen on the wire under a RETIRING nick is held back like a JOIN
    /// under it (the departed puppet's, or the name's new holder's — settled
    /// when the entry resolves) and moves nothing yet; the puppet's own
    /// connection reporting its rename is
    /// [`puppet_renamed`](Self::puppet_renamed), which does move the entry.
    pub fn renamed(&mut self, from: &str, to: &str) -> Vec<HumanEffect> {
        let old = self.fold(from);
        if let Some(entry) = self.retiring_entry(&old) {
            // The holder answers to `to` from here: its story continues
            // under the same entry.
            let new = self.fold(to);
            if new != entry {
                self.aliases.insert(new, (entry.clone(), to.to_string()));
            }
    ...
    /// The puppet's OWN connection reported a rename (the executor's
    /// attempt-tagged event): its nick moves to the new spelling, owned or
    /// retiring, when membership still files it under `connection`; a name a
    /// human took since is not this connection's to move, and nothing
    /// happens. `true` when it moved.
    pub fn puppet_renamed(&mut self, from: &str, to: &str, connection: u64) -> bool {
        ...
        if let Some(arrivals) = self.deferred.remove(&old) {
            // The entry moves; what was held back under the old spelling is
            // still that entry's story, now under the new one.
            self.deferred.insert(new.clone(), arrivals);
        }
        for (entry, _) in self.aliases.values_mut() {
            if *entry == old {
                *entry = new.clone();
            }
        }
    ...
    pub fn set_casemapping(&mut self, cm: CaseMapping) -> Vec<HumanEffect> {
        ...
        // retiring entry's wire spelling, which `retiring` re-derived above.
        ...
        // Aliases re-key by both wire spellings: the name the holder went
        // by, and the retiring entry it belongs to.
        let mut aliases: HashMap<String, (String, String)> = HashMap::new();
        for (old_key, (entry_key, alias_wire)) in std::mem::take(&mut self.aliases) {
            let entry_wire = self
                .retiring
                .values()
                .find(|h| fold_nick(&h.wire, old_cm) == entry_key)
                .map(|h| h.wire.clone());
            let Some(entry_wire) = entry_wire else {
                continue;
            };
            let _ = old_key;
            aliases.insert(
                fold_nick(&alias_wire, cm),
                (fold_nick(&entry_wire, cm), alias_wire),
            );
        }
