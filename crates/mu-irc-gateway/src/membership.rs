...
pub struct Membership {
    ...
    /// barrier — as the name's new holder's, unless the wire's own NICK
    /// settled it first (then what came before was the puppet's, and is
    /// discarded). A chain reported before the first barrier vacates each
    /// spelling in turn, each settled by its own.
    vacating: HashMap<String, Held>,
    /// The spelling a puppet is on the server under by the LAST rename its
    /// connection reported, before the barrier applies it: a JOIN under it
    /// is the puppet's. Folded → (connection, wire); one per connection.
...
impl Membership {
        ...
        S: AsRef<str>,
    {
        ...
        for (key, held) in previous {
            if !self.owned.contains_key(&key) {
                ...
                // connection it was held by, and tombstoned for open syncs
                // so a later snapshot line cannot front it. A reported
                // rename the barrier has not applied keeps its window: the
                // puppet is on the server under the expected name, its
                // QUIT still pending, and what is held back under a
                // vacated spelling is its new holder's, settled at the
                // barrier or by the departure report (panel findings, PR
                // #671 runs 13-15).
    ...
    /// renaming ITSELF: `from` is that connection's, and either nothing is
    /// pending under it or `to` is where a reported rename takes it. A NICK
    /// from a spelling the puppet has moved on from, onto anything else,
    /// is that spelling's new holder's. The bridge asks before letting the
    /// pool follow a NICK.
    ...
    fn move_entry(&mut self, connection: u64, to: &str) {
        ...
        // Never onto a name ANOTHER connection still holds: the server
        // cannot have given this puppet a name it has not taken from the
        // other, so the move is stale and would orphan that entry (PR
        // #679 run 18).
        let taken = self
            .owned
            .get(&new)
            .filter(|_| !self.gone.contains(&new))
            .or_else(|| self.retiring.get(&new))
            .is_some_and(|h| h.connection != connection);
        if taken {
            return;
        }
            ...
            match self.story_key(&key) {
                Some(entry) => {
                    ...
                    if !self.vacating.contains_key(&entry) {
                        ...
                        // resolves its whole window. Its remaining hops
                        // replay as their holders', and nothing of it stays
                        // expected — else its spellings would keep reading
                        // as ours (panel finding, PR #679 run 9).
                ...
                None => {
                    // Under an EXPECTED name: the puppet quit under the
                    // name its last report took it to, before the barrier.
                    // The wire has told the whole story, so the window
                    // settles HERE as the barriers would have — each
                    // vacated spelling's story replayed as its new
                    // holder's, the entry moved to this name, the name
                    // gone from here — and a late barrier finds nothing
                    // (panel findings, PR #671 runs 12-13).
    ...
    pub fn renamed(&mut self, from: &str, to: &str) -> Vec<HumanEffect> {
        ...
        // The puppet's OWN NICK on the wire, for a rename its connection
        // reported and the barrier has not applied — `from` is the
        // connection's, `to` where a reported rename takes it: the puppet
        // shared a channel, so what was held under the old spelling was
        // the puppet's (discarded), and the rename applies now, in order
        // by construction. The entry moves to `to`, and the hop the wire
        // settles this way leaves its barrier nothing.
        if let Some(connection) = self.hop_of(&old) {
            if self.target_of(&new) == Some(connection) {
                ...
                self.move_entry(connection, to);
                // `move_entry` tombstones only the key it moved the entry
                // FROM, and the pool may have caught up: the spelling the
                // wire showed the puppet leave is tombstoned here whatever
                // the entry's key was, or its own pre-rename echo would
                // commit as a human (PR #679 run 14).
                for ch in self.channels.values_mut() {
                    if let Some(sync) = ch.sync.as_mut() {
                        sync.pending.remove(&old);
                        sync.departed.insert(old.clone());
        ...
        // that hop's barrier and of this rename's report. An expected name
        // is the puppet's on the server, so the NICK is nobody else's: the
        // entry and expectation move on to where it takes them, the
        // vacated name is free at once, and the older barrier finds the
        // entry elsewhere (panel finding, PR #679 run 7).
        if !self.vacating.contains_key(&old) {
            if new == old {
                ...
                self.move_entry(connection, to);
                // The spelling the wire showed the puppet leave is
                // tombstoned like any vacated one: the expectation's own
                // eviction covered only the syncs open when the rename was
                // reported, and one opened since would front the puppet's
                // pre-rename echo as a human (panel finding, PR #679 run
                // 8).
                for ch in self.channels.values_mut() {
                    if let Some(sync) = ch.sync.as_mut() {
                        sync.pending.remove(&old);
        ...
        // one), ONTO a spelling ANOTHER connection vacated: the server
        // gave that name away, so its rename went through — the hop
        // settles, its entry moving to the name its report expects. That
        // spelling's own holder is gone, but a name they went ON to is a
        // live fact, replayed as theirs. Then the rename applies as any
        // own NICK does, below; a story's holder is not "own" here (panel
        // findings, PR #679 runs 3 and 5).
        ...
        {
            // A human's NICK ONTO a spelling the puppet has moved on
            // from, before the barrier: the server took it, so it is
            // theirs — but held back as a JOIN under it would be, since
            // the sync still owns the spelling and would evict a human
            // found present under it. So they leave under the old name (a
            // departure in every open sync, and withdrawn where they were
            // present) and arrive under the new one in the story, in each
            // channel the wire had them in — committed or still pending —
            // to be fronted when the hop settles. Their routing memory
            // does not carry across; the window is the price (panel
            // findings, PR #671 run 16 and PR #679 run 1).
    ...
    /// The puppet's OWN connection reported a rename, to be applied at the
    /// bridge's barrier: until then the old spelling is VACATING (still
    /// ours; what the wire says under it is held back) and the new one
    /// EXPECTED (ours on the server already, so a JOIN under it is the
    /// puppet's). A rename reported while one is pending vacates the
    /// intermediate spelling in turn. Nothing when membership no longer
    /// files `from` under `connection`. The effects: a human this
    /// connection still lists under the new spelling is stale, and is
    /// withdrawn.
    pub fn puppet_renaming(&mut self, from: &str, to: &str, connection: u64) -> Vec<HumanEffect> {
        ...
        let old = self.fold(from);
        // A spelling this connection has already VACATED is not a source it
        // can rename from: that report is behind its own later hop, and
        // taking it would leave the connection with two expected names,
        // one of which nothing resolves. (The pool's spelling stays owned
        // while hops are pending, so the owned key does not say this; the
        // window does.) Panel finding, PR #679 run 14.
        if self
            .vacating
            .get(&old)
            .is_some_and(|h| h.connection == connection)
        ...
        let mut effects = Vec::new();
        match self.vacating.get(&new).map(|h| h.connection) {
            Some(c) if c == connection && !self.origin.contains_key(&new) => {
                self.vacating.remove(&new);
                self.forget_story(&new);
                // The hop's own story is its last holder's, who is gone —
                // the server gave the name back — but a name they went ON
                // to is a live fact: replayed as theirs, as this hop's
                // barrier would have, since that barrier will now find
                // nothing pending (panel finding, PR #679 run 16).
                effects = self.settle(connection, Some(&new), true);
            }
            // ANOTHER connection's vacated hop: the server has given that
            // name to this puppet, so that connection's rename went
            // through, and its hop settles here — on the report, not only
            // on a wire NICK, which never comes when the rename happened
            // outside every shared channel (panel finding, PR #679 run
            // 15).
            Some(_) => effects = self.free_hop(&new),
            None => {}
        }
        self.gone.remove(&new);
        effects.extend(self.evict(&new));
    ...
    /// holders moved on to are settled — each story is the spelling's new
    /// holder's (nothing of the puppet's could have come, or the wire's
    /// NICK would have settled it first), replayed under the name they go
    /// by — and the connection's entry moves to the new spelling if
    /// membership still files it under the old. Only a hop still pending
    /// is applied; one the wire, a QUIT under the expected name or the
    /// departure report has settled leaves nothing to move (panel
    /// findings, PR #671 runs 13-15). The effects are the replays.
    ...
    /// The hop `key` belongs to: its name has been given away on the wire,
    /// so that connection's rename went through. Its entry moves on to the
    /// name its report expects and the hop settles, its stories replayed as
    /// their holders' (the vacated spelling's own holder is gone).
    fn free_hop(&mut self, key: &str) -> Vec<HumanEffect> {
        let Some(held) = self.vacating.get(key) else {
            return Vec::new();
        };
        let connection = held.connection;
        // A name a holder went ON to from a hop — a lineage entry, not the
        // hop itself — is given away the same way: the holder is gone, the
        // entry goes, and the hop it descends from is left to its own
        // barrier. Only the hop ITSELF moves the connection's entry on to
        // its expected name (panel finding, PR #679 run 17).
        let lineage = self.origin.remove(key);
        if lineage.is_none() {
            let target = self
                .expected
                .iter()
    ...
    pub fn set_casemapping(&mut self, cm: CaseMapping) -> Vec<HumanEffect> {
        ...
        // Held-back arrivals re-key with the retiring entry they were held
        // under. Two spellings that now fold equal were two NAMES, and two
        // holders' stories cannot become one holder's: the survivor's goes
        // on, the loser's is discarded as an unconfirmed report's is —
        // only that connection's own report could have told them apart,
        // and after the merge it resolves nothing. What the name's holder
        // is doing now, the fresh snapshot says (panel finding, PR #671
        // run 29).
        ...
        for (old_key, held) in retiring {
            ...
            if let Some(story) = self.deferred.remove(&old_key) {
