...
fn join_account_reads_the_extended_join_parameter() {
    ...
    assert_eq!(reg.join_account(&m), Some("cc-3"));
    // `*` is how the server spells "not logged in".
    let m = IrcMessage::parse(":bob!u@h JOIN #c * :Bob");
    assert_eq!(reg.join_account(&m), None);
    // A plain JOIN has no account parameter to read.
