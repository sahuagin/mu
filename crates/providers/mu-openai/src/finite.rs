...
/// calling this).
///
/// This only matters on the INBOUND path, and `CreateResponseRequest` is an
/// OUTBOUND type — we construct requests, we don't receive them — so the live
/// wire never exercises this. It earns its place on two narrow paths: the
/// round-trip / replay test tier (deserializing a logged request back), and the
/// "bug or corruption" backstop. The point is cascade containment: a single
/// non-finite that slips in must die HERE as one coerced-to-absent field, not
/// propagate (a NaN silently breaks every comparison it touches; an unhandled
/// deserialize error fails the whole message → turn → session). Coerce silently
/// and keep going — no logging (library-internal logging is noise, and on a
/// degraded substrate it would drown the signal it pretends to raise).
