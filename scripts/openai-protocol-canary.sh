...
if [ "$alert" = "bead" ]; then
  ...
  body="OpenAI protocol canary on $(hostname) detected: ${failures[*]}. See $log."
  if [ -n "$lib" ]; then
    # shellcheck source=lib/canary-bead.sh
    . "$lib"
    rc=0; filed="$(canary_file_bead openai-protocol-canary-drift "$msg" "$body" openai-protocol-canary)" || rc=$?
    case $rc in
      0) say "bead: $filed" ;;
      ...
      *) say "bead: not filed (no beads client or beadsd url)" ;;
       ...
       && ${tmo:+"$tmo" "${CANARY_BEAD_TIMEOUT:-60}"} beads --url "$url" exec -- create --title "$msg" --slug openai-protocol-canary-drift \
            --type bug --priority P1 --description "$body" --actor openai-protocol-canary >/dev/null 2>&1; then
