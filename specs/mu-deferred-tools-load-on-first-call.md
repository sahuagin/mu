# mu-t4l5e — deferred tool schemas with load-on-first-call
...
1. **Pre-selection at turn start.** Run the existing `capability_hints` ranker
   ...
   never has to ask.
2. **Load-on-first-call.** In `execute_tools.rs`, the `tools.iter().find` miss
...
- `agent/loop_/mod.rs` turn start: pre-selection through
  ...
  names.
- `agent/loop_/execute_tools.rs` ~930-955: load-on-first-call before the
  near-miss branch.
...
- Load-on-first-call hooks the FOUND-tool path in `execute_tools.rs`, not the
